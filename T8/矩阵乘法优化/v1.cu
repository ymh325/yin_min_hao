#include <stdio.h>
#include <chrono> // CPU time
#include <cstring>  
#include <cuda_runtime.h>

__global__ void kernel_mul_v1(float *A,float *B,float *C,int M,int K,int N){
...
int main(){
    ...
    // GPU warm-up 
    kernel_mul_v1<<<grid_z, block_z>>>(A, B, C, M, K, N);
    ...
    cudaEventRecord(kernel_start);  // start timing
    
    for(int i=0; i<test_times; i++) 
    {
        kernel_mul_v1<<<grid_z, block_z>>>(A, B, C, M, K, N);  // launch kernel
    }
    
    cudaEventRecord(kernel_end);      // end timing
