...
__global__ void kernel_mul_v8(float *dA, float *dB, float *dC, int M, int K, int N)
{
    __shared__ float SA[SM * SK * 2];
    __shared__ float SB[SK * SN * 2];
    ...
    //------------
    int ph = 0;
    (float4 &)a[0] = (float4 &)dA[(indA + SA_x) * K + 4 * SA_y + ph * SK];
    for (int id = 0; id < 4; id++)
    {
        if (indA + SA_x >= M || ph * SK + 4 * SA_y + id >= K)
        {
            SA[(4 * SA_y + id) * SM + SA_x] = 0.0f;
        }
        else
        {
            SA[(4 * SA_y + id) * SM + SA_x] = a[id];
        }
    }
    (float4 &)SB[SB_x * SN + 4 * SB_y] = (float4 &)dB[(SB_x + ph * SK) * N + indB + 4 * SB_y];
    
    for (int id = 0; id < 4; id++)
    {

        if (indB + 4 * SB_y + id >= N || SB_x + ph * SK >= K)
        {

            SB[SB_x * SN + 4 * SB_y + id] = 0.0f;
        }
    }
    __syncthreads();
    ...
    {
        //* Moving from dA to SA, moving from dB to SB
        (float4 &)a[0] = (float4 &)dA[(indA + SA_x) * K + 4 * SA_y + ph * SK];
        for (int id = 0; id < 4; id++)
        {
            if (indA + SA_x >= M || ph * SK + 4 * SA_y + id >= K)
            {
                SA[(4 * SA_y + id) * SM + SA_x + ph % 2 * SM * SK] = 0.0f;
            }
            else
            {
                SA[(4 * SA_y + id) * SM + SA_x + ph % 2 * SM * SK] = a[id];
            }
        }
        (float4 &)SB[SB_x * SN + 4 * SB_y + ph % 2 * SN * SK] = (float4 &)dB[(SB_x + ph * SK) * N + indB + 4 * SB_y];
        for (int id = 0; id < 4; id++)
        {
            if (indB + 4 * SB_y + id >= N || SB_x + ph * SK >= K)
            {

                SB[SB_x * SN + 4 * SB_y + id + ph % 2 * SN * SK] = 0.0f;
            }
        }
        //-----------------------------------------------------------------------
        //* Calculate tmp 
        for (int index_k = 0; index_k < SK; index_k++)
        {
            (float4 &)com_a[0] = (float4 &)SA[index_k * SM + threadIdx.x * TM + (ph - 1) % 2 * SM * SK];
            (float4 &)com_a[4] = (float4 &)SA[index_k * SM + threadIdx.x * TM + 4 + (ph - 1) % 2 * SM * SK];
            (float4 &)com_b[0] = (float4 &)SB[index_k * SN + threadIdx.y * TN + (ph - 1) % 2 * SN * SK];
            (float4 &)com_b[4] = (float4 &)SB[index_k * SN + threadIdx.y * TN + 4 + (ph - 1) % 2 * SN * SK];
            for (int row = 0; row < TM; row++)
            {
                for (int col = 0; col < TN; col++)
                {
                    tmp[row * TN + col] += com_a[row] * com_b[col];
                }
            }
        }
    ...
    for (int row = 0; row < TM; row++)
    {
        for (int col = 0; col < TN; col++)
        {
            int i = threadIdx.x * TM + row;
            int j = threadIdx.y * TN + col;
            if (indA + i < M && indB + j < N)
            {
                dC[(indA + i) * N + indB + j] = tmp[row * TN + col];
            }
        }
    }
}
...
int main(){
    ...
    const int BLOCK_X = 16, BLOCK_Y = 16;  // Block size
    const int TM = 8, TN = 8;               // Thread computation size
    ...
    // GPU warm-up 
    kernel_mul_v8<SM, SN, SK, TM, TN><<<grid, block>>>(A, B, C, M, K, N);
    ...
    {
        kernel_mul_v8<SM, SN, SK, TM, TN><<<grid, block>>>(A, B, C, M, K, N);  // launch kernel
    }
